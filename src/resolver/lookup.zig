...
pub const Action = union(enum) {
    ...
    /// Send these bytes to this server as one request, the server being
    ...
    /// call `on_sent`, and hand the answer to `on_request_answer` with `transaction`, the number
    /// of the lookup's transaction, or say the request failed with `on_request_failed`.
...
pub const Lookup = struct {
    ...
    /// The answer to a request over DoH or DoQ, and its `Age` in seconds, zero over DoQ
    ...
    pub fn on_request_answer(
        ...
        return request_module.on_request_answer(self, transaction, message, age_seconds, now_ns);
    }

    /// A request over DoH or DoQ ended without an answer (docs/design.md §22, §23).
    pub fn on_request_failed(self: *Lookup, transaction: u16, now_ns: u64) void {
        request_module.on_request_failed(self, transaction, now_ns);
    }
