//! A lookup over DoH through the fake server (docs/design.md §22): the query's shape, the answer
//! by transaction, the `Age`, the HTTP failure, and a stream's reading of TC and BADCOOKIE.
//! Split from `lookup_https.zig` so the fixtures stay out of it.
...
fn start(harness: *fixtures.Harness, text: []const u8, kind: core.Kind, lookup_seed: u64) !void {
    harness.* = .{ .config = .{ .servers = &servers } };
    try harness.start(text, kind, lookup_seed);
}

/// Polls, requires a DoH request, remembers its message and tells the lookup it went out.
fn send(harness: *fixtures.Harness) !Action {
    const action = harness.poll();
    try testing.expect(action == .send_https);
    harness.query_bytes = action.send_https.message_bytes.len;
    harness.query_body_offset = 0;
...
fn answer(harness: *fixtures.Harness, reply: fixtures.Reply, transaction: u16, age_seconds: u32) Verdict {
    ...
    harness.now_ns += 1;
    return harness.lookup.on_https_answer(transaction, message, age_seconds, harness.now_ns);
}

test "a query over DoH has ID 0, the name as given and no cookie, and is padded to 128 octets" {
    var harness: fixtures.Harness = undefined;
    try start(&harness, "Example.COM.", .a, seed);
    const action = try send(&harness);
    const message = action.send_https.message_bytes;
    try testing.expectEqual(@as(u16, 0), (try wire.header.parse(message)).id);
    try testing.expectEqual(@as(u8, 0), action.send_https.server_index);
    try testing.expectEqual(harness.lookup.transaction.number, action.send_https.transaction);
    ...
    try start(&other, "Example.COM.", .a, seed + 1);
    try testing.expectEqualSlices(u8, message, (try send(&other)).send_https.message_bytes);
}

test "an answer to a transaction the lookup has left is ignored; one to its own is taken" {
    var harness: fixtures.Harness = undefined;
    try start(&harness, "example.com.", .a, seed);
    const first = (try send(&harness)).send_https.transaction;
    // The deadline moves the lookup to the second server and a new transaction.
    harness.now_ns = harness.lookup.deadline_ns - 1;
    const second = (try send(&harness)).send_https;
...
test "a datagram is no answer to a lookup over DoH, whatever it carries" {
    var harness: fixtures.Harness = undefined;
    try start(&harness, "example.com.", .a, seed);
    _ = try send(&harness);
...
test "an Age of 250 turns a TTL of 600 into 350, and an Age past the TTL into 0" {
    ...
    for (cases) |case| {
        var harness: fixtures.Harness = undefined;
        try start(&harness, "example.com.", .a, seed);
        const transaction = (try send(&harness)).send_https.transaction;
...
test "an HTTP failure moves the lookup to the next server and counts one failure" {
    ...
    harness.lookup.on_https_failed(transaction +% 1, harness.now_ns);
    try testing.expectEqual(@as(u8, 0), harness.lookup.server_index);
    harness.lookup.on_https_failed(transaction, harness.now_ns);
    try testing.expectEqual(@as(u8, 1), harness.lookup.server_index);
    try testing.expectEqual(@as(u8, 1), harness.servers.failures(0));
    try testing.expectEqual(@as(u8, 1), (try send(&harness)).send_https.server_index);
}

test "over DoH a truncated answer is read as it stands, and BADCOOKIE fails the server" {
    var harness: fixtures.Harness = undefined;
    try start(&harness, "example.com.", .a, seed);
    var transaction = (try send(&harness)).send_https.transaction;
