//! A table over DoH servers (docs/design.md §22, §23): an exchange handed out with its
//! transaction, its answer taken by handle, and an exchange that failed moving the lookup on.
...
test "a table hands out an exchange, and takes its answer by handle and transaction" {
    var table: fixtures.Table = .{ .config = .{ .servers = &servers } };
    table.open();
    const handle = try table.start("example.com.");
    const request = table.poll().?.action.send_exchange;
    ...
    try testing.expectEqual(Verdict.ignored, table.resolver.on_exchange_answer(handle, request.transaction +% 1, message, 0, table.now_ns));
    try testing.expectEqual(Verdict.accepted, table.resolver.on_exchange_answer(handle, request.transaction, message, 0, table.now_ns));
    try testing.expectEqual(@as(usize, 1), table.poll().?.action.done.addresses.len);
}

test "an exchange that failed in a table offers the lookup to the next server" {
    var table: fixtures.Table = .{ .config = .{ .servers = &servers } };
    table.open();
    const handle = try table.start("example.com.");
    const first = table.poll().?.action.send_exchange;
    table.resolver.on_sent(handle, table.now_ns);
    try testing.expectEqual(@as(?Event, null), table.poll());
    table.resolver.on_exchange_failed(handle, first.transaction, table.now_ns);
    const second = table.poll().?.action.send_exchange;
