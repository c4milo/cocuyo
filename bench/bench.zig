...
//! reader knows how much of every other row is the loop and not the operation. Nothing here reads
//! a fixture at comptime: every input is copied into a `var` at setup, so the optimizer sees a
//! runtime value and cannot fold the operation away. Every result goes through
//! `std.mem.doNotOptimizeAway` for the same reason.
//!
...
/// so the width is checked against every case at comptime rather than discovered in the output.
const name_width = 60;
...
pub fn main() void {
    std.debug.print(
        "cocuyo bench: {t}, {s}-{s}, {d} samples per case, clock {s} stepping {d} ns, ns per operation\n\n",
        .{
            builtin.mode,
...
fn sample(case: Case) u64 {
    const start = now_ns();
    var run: u32 = 0;
    while (run < case.iterations) : (run += 1) case.run();
    const elapsed = now_ns() - start;
    assert(run == case.iterations);
