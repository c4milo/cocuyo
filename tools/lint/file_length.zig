...
//!
//! Over every `.zig` and `.sh` file under `src/`, `tools/`, `build/`, `examples/`, `bench/` and
//! `io/`, and every model under `spec/`, Lean's `.lean` and TLA+'s `.tla`, the rule counts lines
//! the way an editor numbers them — one per newline, plus one for a last line with no newline —
//! and reports a file over the limit once, at the first line past it. A model is hand-written like
//! the code, and until 2026-09-24 the rule did not read one: `LookupProofs.lean` had reached 549
//! lines.
//!
...
pub const config: file_length.Config = .{
    .scope = .{
        .extensions = &.{ ".zig", ".sh", ".lean", ".tla" },
        .include_directories = &.{ "src", "tools", "build", "examples", "bench", "io", "spec" },
    },
    .max_lines = max_lines,
...
test "file-length reads the models, and flags one over the limit" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    try testing.expect(config.scope.applies("spec/lean/Spec/LookupProofs.lean"));
    try testing.expect(config.scope.applies("spec/tla/engine/Engine.tla"));
    try testing.expect(!config.scope.applies("spec/tla/engine/Engine_tcp_1_1_bound_4_1.cfg"));
    try testing.expect(!config.scope.applies("spec/README.md"));
    const findings = try harness.run(arena, Rule, "spec/lean/Spec/LookupProofs.lean", failing_fixture);
    try harness.expect_messages(findings, &.{"501 lines, over the 500-line limit; split the file"});
}
