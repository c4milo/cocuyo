...
//!
//! Run:  zig build lint, which passes `--max 15` and build.zig, build, src, tools and examples.
//!
//! The scorer is pepegrillo's: SonarSource Cognitive Complexity mapped onto Zig, with every
//! function and every `test` block scored, one line per declaration over the threshold, in the
//! shape the Zig compiler prints an error,
//!
