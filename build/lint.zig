...
//! the same rules over a canary tree. build.zig stays short (CLAUDE.md, Layout), so the wiring is
//! in this file.
//!
...
/// The cognitive-complexity threshold of CLAUDE.md (Conventions). Never raised: a function over it
/// is split.
...
const canary_source =
    ...
    \\}
    \\pub fn ordering(path: []const u8) !void {
...
fn add_canary_tree(b: *std.Build) std.Build.LazyPath {
    ...
    _ = tree.add("src/resolver/canary.zig", canary_source);
